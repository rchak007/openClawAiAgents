...
<style>
/* ═══════════════════════════════════════════════════════════════════════════
   DESIGN SYSTEM — "Editorial Craft"
   Warm, papery light mode default. Rich ink dark mode.
   Notion calm + Linear precision + Figma playfulness
   ═══════════════════════════════════════════════════════════════════════════ */
...
[data-theme="dark"] .theme-toggle::after {
  content: '🌙'; transform: translateX(23px);
}
...
.btn-primary {
  ...
  box-shadow: 0 1px 4px rgba(224, 122, 58, 0.25);
}
...
.search-wrap input:focus {
  background: var(--bg-input); border-color: var(--border-focus);
...
.item-text {
  ...
  flex: 1; color: var(--text-primary); line-height: 1.45;
  transition: all var(--transition);
}
.item.done .item-text {
  text-decoration: line-through; text-decoration-color: var(--text-tertiary);
...
.modal input, .modal select {
  ...
  border-radius: var(--radius); color: var(--text-primary);
  font-family: var(--font-display); font-size: 14px;
  outline: none; margin-bottom: 14px; transition: all var(--transition);
}
...
.empty h3 { font-size: 20px; font-weight: 700; margin-bottom: 8px; letter-spacing: -0.4px; }
...
.toast {
  padding: 11px 18px; background: var(--bg-surface);
...
.tree-node:nth-child(1) { animation-delay: 30ms; }
.tree-node:nth-child(2) { animation-delay: 60ms; }
.tree-node:nth-child(3) { animation-delay: 90ms; }
.tree-node:nth-child(4) { animation-delay: 120ms; }
.tree-node:nth-child(5) { animation-delay: 150ms; }
...
@media (max-width: 640px) {
  ...
  .modal { min-width: auto; margin: 16px; border-radius: var(--radius-lg); }
  .node-actions, .item-actions { opacity: 1; pointer-events: auto; }
  .node-children { margin-left: 10px; padding-left: 10px; }
}
</style>
</head>
<body>
...
function toast(msg, type = 'success') {
  const id = ++toastId;
  toasts.push({ id, msg, type }); render();
  setTimeout(() => { toasts = toasts.filter(t => t.id !== id); render(); }, 3000);
}

function countItems(node) {
  let total = 0, done = 0;
  (node.items || []).forEach(i => { total++; if (i.done) done++; });
  Object.values(node.children || {}).forEach(c => {
    const r = countItems(c); total += r.total; done += r.done;
  });
  return { total, done };
}

function getTotalStats() {
  let total = 0, done = 0, nodes = 0;
  (function walk(obj) {
  ...
  return { total, done, pending: total - done, nodes };
}

function extractTags(text) { return text.match(/\b[A-Z][A-Z0-9]{1,9}\b/g) || []; }
function matchesSearch(text) { return !searchQuery || text.toLowerCase().includes(searchQuery.toLowerCase()); }
...
function expandAll() {
  (function walk(nodes, path) {
    Object.entries(nodes).forEach(([name, node]) => {
      const p = [...path, name]; expandedNodes.add(pathKey(p));
...
async function addItem(path, text) {
  if (!text.trim()) return;
  try { await api('/api/item/add', { path, text: text.trim() }); toast('Item added'); expandedNodes.add(pathKey(path)); await loadTodos(); }
  catch (e) { toast(e.message, 'error'); }
}
async function editItem(path, itemId, updates) {
  try { await api('/api/item/edit', { path, item_id: itemId, ...updates }); await loadTodos(); }
  catch (e) { toast(e.message, 'error'); }
}
async function deleteItem(path, itemId) {
  try { await api('/api/item/delete', { path, item_id: itemId }); toast('Deleted'); await loadTodos(); }
  catch (e) { toast(e.message, 'error'); }
}
...
async function deleteNode(path) {
  try { await api('/api/node/delete', { path }); toast('Folder deleted'); await loadTodos(); }
  catch (e) { toast(e.message, 'error'); }
}
async function renameNode(path, newName) {
  if (!newName.trim()) return;
  try { await api('/api/node/rename', { path, new_name: newName.trim() }); toast('Renamed'); await loadTodos(); }
  catch (e) { toast(e.message, 'error'); }
}
...
function showModal(type, props = {}) {
  modal = { type, props }; render();
  setTimeout(() => { const el = document.querySelector('.modal input, .modal select'); if (el) el.focus(); }, 60);
}
...
function renderTags(text) {
  ...
  return '<div class="item-tags">' + tags.map(t => {
    const special = ['URGENT','ASAP','CRITICAL','HABIT','WEEKLY','DAILY','GOAL','MILESTONE'].includes(t);
...
function renderItem(item, path) {
  ...
  return `<div class="item ${item.done?'done':''}">
    ...
    <div class="item-actions">
      ...
      <button class="btn-icon" onclick="showModal('moveItem',{path:${jp},item:${ji}})" title="Move">↗</button>
      <button class="btn-icon danger" onclick="showModal('confirm',{msg:'Delete item #${item.id}?',action:'deleteItem',args:[${jp},${item.id}]})" title="Delete">×</button>
    </div>
  </div>`;
}
...
function renderNode(name, node, parentPath) {
  ...
  const pk = pathKey(path), isOpen = expandedNodes.has(pk);
  const { total, done } = countItems(node);
  ...
  return `<div class="tree-node">
    <div class="node-header" onclick="toggleNode(${jp})">
      ...
      <div class="node-actions" onclick="event.stopPropagation()">
        ...
        <button class="btn-icon danger" onclick="showModal('confirm',{msg:'Delete \\'${esc(name).replace(/'/g,"\\'")}\\' and all contents?',action:'deleteNode',args:[${jp}]})" title="Delete">×</button>
      </div>
    </div>
...
function renderModal() {
  ...
  if (type === 'addItem') {
    const jp = JSON.stringify(props.path);
  ...
  if (type === 'moveItem') {
    ...
    (function collect(nodes, p) {
      Object.entries(nodes).forEach(([name, node]) => {
        const fp = [...p, name]; paths.push(fp);
        if (node.children) collect(node.children, fp);
      });
    })(todoData.nodes || {}, []);
    const jp = JSON.stringify(props.path);
    return `<div class="modal-overlay" onclick="if(event.target===this)closeModal()"><div class="modal">
      ...
      <select id="md">${paths.map(p => `<option value='${JSON.stringify(p)}' ${pathKey(p)===pathKey(props.path)?'selected':''}>${esc(p.join(' › '))}</option>`).join('')}</select>
  ...
  if (type === 'confirm') {
    return `<div class="modal-overlay" onclick="if(event.target===this)closeModal()"><div class="modal">
      ...
      <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancel</button><button class="btn btn-danger" onclick="${props.action}(${props.args.map(a=>JSON.stringify(a)).join(',')});closeModal();">Delete</button></div>
    </div></div>`;
  }
  return '';
}
...
function render() {
  ...
  document.getElementById('app').innerHTML = `
    ...
    <div class="tree-container">
      ${tree || `<div class="empty">
        <div class="empty-icon">🌱</div>
        <h3>Your tree is empty</h3>
        <p>Create a root folder to start organizing</p>
        <button class="btn btn-primary" onclick="showModal('addRootFolder')">+ Create Folder</button>
      </div>`}
    </div>
    ${renderModal()}
